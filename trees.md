...
The binary search is able to sort data in **O log(n) time**, so its efficiency actually _increases_ as the input size increases. This occurs because the data always gets split in half, so significantly large inputs simply result in significant amounts of data exclusion as well.
